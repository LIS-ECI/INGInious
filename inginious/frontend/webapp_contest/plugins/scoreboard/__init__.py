...
class ScoreBoardCourse(INGIniousAuthPage):
    ...
    def GET_AUTH(self, courseid):  # pylint: disable=arguments-differ
        ...
        return self.template_helper.get_custom_renderer('frontend/webapp_contest/plugins/scoreboard').main(course, names)
...
class ScoreBoard(INGIniousAuthPage):
    ...
    def GET_AUTH(self, courseid, scoreboardid):  # pylint: disable=arguments-differ
        ...
        renderer = self.template_helper.get_custom_renderer('frontend/webapp_contest/plugins/scoreboard')
...
def course_menu(course, template_helper):
    ...
    if scoreboards != []:
        return str(template_helper.get_custom_renderer('frontend/webapp_contest/plugins/scoreboard', layout=False).course_menu(course))
    else:
        return None
...
def task_menu(course, task, template_helper):
    ...
    try:
        ...
        if tolink:
            return str(template_helper.get_custom_renderer('frontend/webapp_contest/plugins/scoreboard', layout=False).task_menu(course, tolink))
        return None
    except:
        return None
...
def init(plugin_manager, _, _2, _3):
    """
        ...
        ::
            ...
            - plugin_module: "inginious.frontend.webapp_contest.plugins.scoreboard"
