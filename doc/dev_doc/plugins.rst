...
- ``plugin_config`` which is a dictionary containing the plugin configuration fields set in your ``configuration.yaml``
  ...
  ::
        ...
        plugins:
            - plugin_module: inginious.frontend.webapp_contest.plugins.demo
              param1: "value1"

  will generate the following ``plugin_config`` dictionary :
  ::

        {"plugin_module": "inginious.frontend.webapp_contest.plugins.demo", "param1": "value1"}
...
``course_accessibility`` (``course``, ``default``)
    Returns: inginious.frontend.webapp_contest.accessible_time.AccessibleTime
